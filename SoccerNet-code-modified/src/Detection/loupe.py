...
class DETR(PoolingBaseModel):
    def __init__(self, feature_size, max_samples, cluster_size, output_dim,
        ...
        super(DETR, self).__init__(
            ...
            add_batch_norm=add_batch_norm,
            is_training=is_training)
        self.model = detr.detr_resnet50(pretrained=True)
        self.in_features = self.model.class_embed.in_features
        self.model.class_embed = nn.Linear(in_features=self.in_features, out_features=output_dim)

    def forward(self, reshaped_input):
        reshaped_input = reshaped_input.permute(0, 2, 1)  # PyTorch expects (batch, channels, length)
        x = self.model(reshaped_input)
