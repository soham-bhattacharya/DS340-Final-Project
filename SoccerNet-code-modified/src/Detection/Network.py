...
class networkMinutes():
    ...
    def __init__(self, dataset, network_type="CNN", VLAD_k=64, VLAD_gating=True, VLAD_batch_norm=True):
        ...
        with tf.name_scope('network'):
            ...
            elif ("DETR" in network_type.upper()):
                print("Using DETR")
                DETR = DETRModel(feature_size=512, max_samples=dataset.number_frames_in_window, cluster_size=VLAD_k, 
                                output_dim=512, gating=VLAD_gating, add_batch_norm=VLAD_batch_norm,
                                is_training=True)
                x = tf.reshape(x, [-1, 512])
                x = DETR.forward(x)
...
class DETR_Network():
    def __init__(self, dataset, network_type="DETR", VLAD_k=64, VLAD_gating=True, VLAD_batch_norm=True):
        tf.set_random_seed(1234)
        ...
        with tf.name_scope('network'):
            ...
            if ("DETR" in network_type.upper()):
                print("Using DETR")
                self.detr = DETR(feature_size=512, max_samples=dataset.number_frames_in_window, cluster_size=VLAD_k, 
                         output_dim=512, gating=VLAD_gating, add_batch_norm=VLAD_batch_norm,
                         is_training=True)
                x = tf.reshape(x, [-1, 512])
                x = self.detr.forward(x)

            # Add other elif blocks for other models here
